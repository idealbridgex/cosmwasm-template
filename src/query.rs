...
pub fn query_counter(deps: Deps, _env: Env) -> StdResult<Binary> {
    ...
    let resp = to_json_binary(&CounterResponse { counter }).unwrap();
    Ok(resp)
}
...
mod tests {
    ...
    fn test_query() {
        ...
        let expect_data_0 = to_json_binary(&CounterResponse { counter: 0 }).unwrap();
        let expect_data_1 = to_json_binary(&CounterResponse { counter: 1 }).unwrap();
